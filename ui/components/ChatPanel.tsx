...
interface Props {
  serverName: string
  threadId: string
  onMessage: (server: string, preview: string) => void
}
...
export default function ChatPanel({ serverName, threadId, onMessage }: Props) {
  ...
  useEffect(() => {
    setMessages([{
      ...
      content: `Connected to **${serverName}**. Ready for your commands.`,
      ts: new Date(),
    }])
  }, [serverName])
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      if (q.toLowerCase().includes("full sweep") || q.toLowerCase().includes("run sweep")) {
        result = await runSweep(serverName)
      ...
      addMsg({ role: "agent", content: `Error: ${msg}`, severity: "critical", ts: new Date() })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
      ...
      <div style={{
        ...
        background: "rgba(5,5,8,0.6)", backdropFilter: "blur(12px)",
      }}>
        <div style={{
        ...
        <div style={{
          marginTop: "6px", fontSize: "10px", color: "#1e293b", textAlign: "center",
          fontFamily: "\'JetBrains Mono\', monospace",
        }}>
  ...
  if (msg.thinking) {
    return (
      <div style={{ padding: "6px 20px", display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px" }}>
        ...
        <div style={{ display: "flex", gap: "4px", alignItems: "center", padding: "10px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "4px 12px 12px 12px" }}>
          {[0,1,2].map(i => (
            <div key={i} style={{
        ...
        <style>{`
          @keyframes swbounce { 0%,60%,100%{transform:translateY(0)} 30%{transform:translateY(-6px)} }
        `}</style>
      </div>
    )
  }
      ...
      <div style={{ maxWidth: "72%", minWidth: "60px" }}>
        {sev && msg.severity !== "healthy" && (
          <div style={{
            ...
            padding: "2px 8px", borderRadius: "4px", marginBottom: "5px",
        ...
        <div style={{
          fontSize: "10px", color: "#1e293b", marginTop: "3px",
          textAlign: isUser ? "right" : "left",
          fontFamily: "\'JetBrains Mono\', monospace",
        }}>{msg.ts.toLocaleTimeString()}</div>
      </div>
      {isUser && (
        <div style={{
