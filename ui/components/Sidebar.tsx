...
const STATUS_COLOR: Record<string, string> = {
  healthy: "#10b981", warn: "#f59e0b", critical: "#ef4444"
}
...
export default function Sidebar({ servers, activeServer, onServerChange, chatHistory, activeChat, onChatSelect, status }: Props) {
  return (
    <div style={{
      ...
      background: "rgba(8,8,12,0.9)", position: "relative", zIndex: 1,
    }}>
      <div style={{ padding: "16px 16px 12px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div>
            <div style={{ fontSize: "14px", fontWeight: 800, color: "#f1f5f9", letterSpacing: "-0.3px" }}>SysWatcher</div>
            <div style={{ fontSize: "10px", color: "#475569", fontFamily: "\'JetBrains Mono\', monospace", letterSpacing: "0.5px" }}>AI MONITOR</div>
          </div>
        </div>
      </div>
          ...
          return (
            <button key={s} onClick={() => onServerChange(s)} style={{
              ...
              <div style={{
                ...
                background: sColor, boxShadow: isActive ? `0 0 6px ${sColor}` : "none", flexShrink: 0,
              }} />
      ...
      <div style={{ padding: "8px 12px", flex: 1, overflowY: "auto" }}>
        ...
        {chatHistory.length === 0 ? (
          <div style={{ padding: "12px 8px", fontSize: "11px", color: "#1e293b", fontFamily: "\'JetBrains Mono\', monospace", textAlign: "center" }}>
            No history yet
          </div>
        ) : (
          chatHistory.map(h => (
      ...
      {status && (
        <div style={{ padding: "10px 12px", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px" }}>
            {[
              { label: "CRITICAL", value: status.critical_count, color: "#ef4444" },
              { label: "WARN", value: status.warn_count, color: "#f59e0b" },
            ].map(m => (
